...
export function estimateTotalSourceFiles(): number {
  return countCodexSessionFiles() + countClaudeSessionFiles() + countCopilotSessionFiles() + countGeminiSessionFiles();
}
...
export function syncAll(onProgress?: (progress: SyncProgress) => void): SyncStats {
  ...
  return {
    scannedFiles: codexStats.scannedFiles + claudeStats.scannedFiles + copilotStats.scannedFiles + geminiStats.scannedFiles,
    updatedSessions: codexStats.updatedSessions + claudeStats.updatedSessions + copilotStats.updatedSessions + geminiStats.updatedSessions,
    skippedFiles: codexStats.skippedFiles + claudeStats.skippedFiles + copilotStats.skippedFiles + geminiStats.skippedFiles,
    warnings: codexStats.warnings + claudeStats.warnings + copilotStats.warnings + geminiStats.warnings,
    warningDetails: [
